...
/** 浏览器上报的画布快照。 */
export type CanvasSnapshot = { nodes?: unknown[]; edges?: unknown[]; viewport?: unknown; [key: string]: unknown };
...
export class CanvasSession {
    ...
    private clientStates = new Map<string, { snapshot: CanvasSnapshot; tools: CanvasToolDefinition[] }>();
    ...
    updateState(clientId: string, body: { snapshot?: CanvasSnapshot; tools?: CanvasToolDefinition[] }) {
    ...
    activateClient(clientId: string) {
        if (this.clients.has(clientId) || this.clientStates.has(clientId)) this.activeClientId = clientId;
    ...
    callTool(name: string, input: Record<string, unknown>): Promise<unknown> {
        if (!this.hasCanvas()) return Promise.reject(new Error("当前没有已连接画布，请先在浏览器打开画布并连接本地 Agent"));
        const toolCallId = crypto.randomUUID();
        const response = this.clients.get(this.activeClientId);
        if (!response) return Promise.reject(new Error("画布客户端连接已断开，请重新连接"));
    ...
    /** 浏览器回写工具执行结果。 */
    resolveResult(clientId: string, body: { toolCallId?: string; result?: unknown; error?: string }): boolean {
        ...
        if (body.error) call.reject(new Error(body.error));
    ...
    broadcast(type: string, payload: unknown) {
        for (const res of this.clients.values()) writeEvent(res, type, payload);
    }
...
/** 将事件写入 SSE 响应。 */
function writeEvent(res: Response, type: string, payload: unknown) {
