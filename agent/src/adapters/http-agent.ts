...
export class HttpAgentAdapter implements AgentAdapter {
    ...
    constructor(private config: HttpAgentConfig) {}

    async start() {
        logger.info("Http agent adapter ready", { model: this.config.model });
    }

    async startThread(cwd?: string): Promise<ThreadBrief> {
        const id = `http-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        return { id, preview: "", name: "新会话", cwd };
    }

    async resumeThread(threadId: string, cwd?: string): Promise<ThreadBrief> {
    ...
    async runTurn(prompt: string, emit: AgentEmit, _attachments: AgentAttachment[], options: TurnOptions) {
        options.onStart?.();
