...
import { DEFAULT_PORT, loadConfig, saveConfig, VERSION } from "../config.js";
...
export function startHttpServer() {
    ...
    const requestClientId = (req) => String(req.query.clientId || req.body?.clientId || sseClientId || "default");
    /** 将 Agent 事件广播到全部已连接网页。 */
    const emit = (type, payload) => canvasSession.broadcast(type, payload);
    const app = express();
    ...
    app.use((req, res, next) => {
        ...
        if (!origin || req.method === "OPTIONS" || req.path === "/health" || req.path === "/config")
            return next();
        config.origins ||= [];
    ...
    app.use((req, res, next) => {
        if (validToken(req, config.token))
            return next();
    ...
    app.post("/api/tools", route(async (req, res) => {
        ...
        if (!name)
            return res.status(400).json({ ok: false, error: "工具名称不能为空" });
    ...
    app.post("/agent/turn", route(async (req, res) => {
        ...
        if (!threadId)
            return res.status(409).json({ ok: false, error: "请先创建会话线程" });
    ...
    app.post("/agent/approval", route(async (req, res) => {
        ...
        const ok = await adapter.resolveApproval(String(req.body?.requestId || ""), decision);
        res.status(ok ? 200 : 409).json({ ok });
    }));
    ...
    app.post("/agent/adapters", route(async (req, res) => {
        ...
        if (!id || !baseUrl || !model)
            return res.status(400).json({ ok: false, error: "id/baseUrl/model 不能为空" });
        if (adapterRegistry.addHttpAgent(id, { baseUrl, model, apiKey: String(req.body?.apiKey || "") || undefined }))
            return res.status(201).json({ ok: true, adapters: adapterRegistry.list() });
...
function requireClient(req) {
    ...
    if (id && !canvasSession.hasCanvas())
        throw new Error("当前没有已连接画布，请先在浏览器打开画布并连接本地 Agent");
}
