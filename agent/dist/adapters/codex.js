...
export class CodexAdapter {
    id = "codex";
    label = "Codex";
    capabilities = ["threads", "approvals", "streaming", "skills", "models"];
    clientPromise = null;
    emitRef = () => undefined;
    async start(emit) {
        this.emitRef = emit;
    ...
    async listThreads(opts) {
        ...
        const data = Array.isArray(field(result, "data")) ? field(result, "data").map(summarizeThread) : [];
        return { data };
    }
    ...
    async runTurn(prompt, emit, attachments, options) {
        const client = await this.getClient(emit);
        const threadId = options.threadId || "";
        if (!threadId)
            throw new Error("Codex 需要先创建或恢复线程");
    ...
    async listModels() {
        ...
        const data = Array.isArray(result) ? result : Array.isArray(field(result, "data")) ? field(result, "data") : [];
