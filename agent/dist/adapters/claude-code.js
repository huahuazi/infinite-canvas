...
export class ClaudeCodeAdapter {
    ...
    sessionRecord(sessionId) {
        try {
            const value = JSON.parse(fs.readFileSync(path.join(SESSION_DIR, `${sessionId}.json`), "utf8"));
            return value;
        }
        catch {
            return null;
    ...
    async resumeThread(threadId, cwd, _permissionMode) {
        ...
        if (!session)
            throw new Error(`找不到 Claude 会话 ${threadId}`);
    ...
    async listThreads(opts) {
        ...
        return { data: filtered.map((item) => ({ id: item.id, preview: item.preview, name: null, updatedAt: item.updatedAt })) };
    }
