...
export class CodexEventHistory {
    ...
    constructor(file = CODEX_EVENT_HISTORY_FILE) {
        this.file = file;
    }
    ...
    removeThread(threadId) {
        return this.run(async () => {
            ...
            if (items.length === data.items.length && turns.length === data.turns.length)
                return;
            const nextData = { version: 1, items, turns };
            await this.save(nextData);
            this.data = nextData;
        });
    }
