...
export class CodexAppClient {
    ...
    completedTurnResults = new Map();
    pendingDeltas = new Map();
    startedItems = new Map();
    itemSequences = new Map();
    nextItemSequences = new Map();
    plansByTurn = new Map();
    approvalRequests = new Map();
    finalizingTurns = new Map();
    skillReloads = new Map();
    silentThreadIds = new Set();
    structuredOutputByTurn = new Map();
    pendingSilentThreadStarts = new Set();
    pendingThreadStartedNotifications = [];
    pendingPreheatThreadStarts = 0;
    preheatingThreadIds = new Set();
    failing = false;
    failureMessage = "";
    /** 保存 app-server 子进程和事件出口。 */
    constructor(child, emit, eventHistory = codexEventHistory) {
        this.child = child;
        this.emit = emit;
    ...
    /** 创建新的 Codex 线程。 */
    async startThread(cwd, permissionMode = "request", preheat = false) {
        if (preheat)
            this.pendingPreheatThreadStarts += 1;
        let threadId = "";
        try {
    ...
    async resumeThread(threadId, cwd, permissionMode = "request", preheat = false) {
        ...
        try {
            ...
            if (!thread.id)
                throw new Error("Codex app-server 没有返回 thread id");
            if (preheat)
                await this.completeMcpPreheat(thread.id);
            return thread;
        }
        finally {
            if (preheat)
    ...
    async completeMcpPreheat(threadId) {
    ...
    async closeSkillDraftThread(threadId) {
        try {
            if (!this.failing)
                await this.request("thread/unsubscribe", { threadId }, true);
        }
        finally {
    ...
    listSkills(cwd, forceReload = false) {
        if (!forceReload)
            return this.request("skills/list", { cwds: [cwd] });
        ...
        const clear = () => {
            if (this.skillReloads.get(key) === reload)
                this.skillReloads.delete(key);
        };
    ...
    clearPlanUpdates(threadId) {
        this.plansByTurn.forEach((item, key) => {
            if (item.threadId === threadId)
                this.plansByTurn.delete(key);
        });
    }
    /** 启动一个 Codex turn 并等待完成通知。 */
    async startTurn(threadId, prompt, images, permissionMode, model, effort, onTurn, skill, messageText, outputSchema) {
        this.preheatingThreadIds.delete(threadId);
        this.currentThreadId = threadId;
        this.currentTurnId = "";
        this.lastUsage = null;
        const pendingStart = { threadId, prompt, messageText, onTurn };
        ...
        try {
            ...
            if (this.pendingTurnStart === pendingStart)
                this.pendingTurnStart = undefined;
            if (pendingStart.turnId)
                this.startedTurnKeys.delete(turnCacheKey(threadId, pendingStart.turnId));
        }
    }
    /** 在静默线程中生成结构化输出。 */
    async generateSkillDraft(threadId, prompt, outputSchema, model, effort) {
        ...
        if (!output)
            throw new Error("Codex 没有返回 Skill 草稿");
        return output;
    }
    /** 中断当前正在运行且属于指定线程的 Codex turn。 */
    async interruptCurrentTurn(requestedThreadId) {
        const threadId = this.currentThreadId;
        const turnId = this.currentTurnId;
        if (!threadId || !turnId || (requestedThreadId && requestedThreadId !== threadId))
            return false;
        try {
    ...
    async startSilentThread(method, params) {
        ...
        try {
            ...
            if (!this.pendingSilentThreadStarts.size)
                this.flushPendingThreadStartedNotifications();
        }
    }
    /** 发送 JSON-RPC 请求并保存待处理 Promise。 */
    request(method, params, silent = false) {
        if (this.failing)
            return Promise.reject(new CodexReportedError(this.failureMessage || "Codex app-server 已停止"));
        const id = this.nextId++;
        this.write({ id, method, params }, silent);
        return new Promise((resolve, reject) => this.pending.set(id, { resolve: (result) => resolve(result), reject, silent }));
    }
    /** 发送无需响应的 JSON-RPC 通知。 */
    ...
    handleNotification(method, params) {
        if (this.handleSilentNotification(method, params))
            return;
        ...
        if (method === "turn/plan/updated") {
            ...
            if (update.threadId && update.turnId)
                this.plansByTurn.set(turnCacheKey(update.threadId, update.turnId), update);
            params = update;
        }
        ...
        if (event.type === "item.started" || event.type === "item.completed") {
            ...
            if (id && event.type === "item.completed") {
                ...
                if (started)
                    event.item = mergeDefinedRecord(started, item || {});
        ...
        if (event.type === "item.completed") {
            ...
            if ((item?.type === "agent_message" || item?.type === "plan") && streamedText && !String(item.text || "").trim())
                item.text = streamedText;
            if (item?.type === "reasoning" && streamedText && !readableCodexText(field(item, "summary")))
                item.summary = streamedText;
            if (item?.type === "command_execution" && streamedText && !String(item.aggregatedOutput || "").trim())
                item.aggregatedOutput = streamedText;
            const threadId = String(field(event, "thread_id") || this.currentThreadId);
            const turnId = String(field(event, "turn_id") || this.currentTurnId);
        ...
        if (event.type === "turn.completed") {
            ...
            if (plan)
                this.plansByTurn.set(planKey, { ...plan, turnStatus: String(field(turn, "status") || "completed") });
            if (threadId && turnId) {
                ...
                const turnRecord = {
                    ...(turn && typeof turn === "object" && !Array.isArray(turn) ? turn : { id: turnId, status: field(turn, "status") || "completed" }),
        ...
        if (event.type === "turn.completed") {
            ...
            if (turnPersistence)
                void turnPersistence.then(complete);
            else
                complete();
            return;
        }
    ...
    handleSilentNotification(method, params) {
        ...
        if (!(method === "error" || method.startsWith("turn/") || method.startsWith("item/") || method === "thread/tokenUsage/updated"))
            return false;
        const threadId = String(field(params, "threadId") || this.currentThreadId);
        if (!threadId || !this.silentThreadIds.has(threadId))
            return false;
        const turnId = String(field(params, "turnId") || field(field(params, "turn"), "id") || this.currentTurnId);
        if (method === "turn/started") {
            this.currentThreadId = threadId;
        ...
        if (method === "item/completed") {
            ...
            if (item.type === "agent_message" && turnId) {
                ...
                if (text)
                    this.structuredOutputByTurn.set(turnCacheKey(threadId, turnId), text);
            }
            if (itemId)
                this.textByItem.delete(key);
            return true;
    ...
    flushPendingThreadStartedNotifications() {
    ...
    completeTurn(event, params, eventScope) {
    ...
    flushDelta(key) {
        ...
        if (pending.delta)
            this.emit("agent_event", { agent: "codex", type: "item.updated", item: { id: pending.params.itemId, type: pending.itemType, delta: pending.delta }, ...codexEventScope(pending.params) });
    }
    /** 按 Codex summaryIndex 保存 reasoning 分段，顺序与线程历史一致。 */
    appendReasoningText(key, params) {
        const segments = this.reasoningTextByItem.get(key) || new Map();
    ...
    finishTurnDeltas(threadId, turnId) {
    ...
    assignItemSequence(threadId, turnId, itemId) {
        const key = itemCacheKey({ threadId, turnId, itemId });
    ...
    notifyTurnStarted(threadId, turnId, fallback) {
        if (!threadId || !turnId)
            return;
        ...
        if (registration && !registration.turnId)
            registration.turnId = turnId;
        const key = turnCacheKey(threadId, turnId);
        if (this.startedTurnKeys.has(key))
            return;
        if (!registration)
            return;
    ...
    failAll(message, reported = false) {
        if (this.failing)
            return;
        this.failing = true;
        ...
        this.activeTurns.forEach(({ threadId, turnId, prompt, messageText }, key) => {
            if (this.silentThreadIds.has(threadId))
                return;
            if (!this.finalizingTurns.has(key))
                failedTurns.set(key, { threadId, turnId, prompt, messageText });
        });
        ...
        if (pendingStart?.turnId && !this.silentThreadIds.has(pendingStart.threadId)) {
            ...
            if (!this.finalizingTurns.has(key) && !failedTurns.has(key))
                failedTurns.set(key, { threadId: pendingStart.threadId, turnId: pendingStart.turnId, prompt: pendingStart.prompt, messageText: pendingStart.messageText });
        }
...
function readableCodexText(value) {
    if (typeof value === "string")
        return value.trim();
    if (Array.isArray(value))
        return value.map(readableCodexText).filter(Boolean).join("\n");
    if (!value || typeof value !== "object")
        return "";
    return readableCodexText(field(value, "text"));
}
/** 合并单个 reasoning 分段增量并按 summaryIndex 输出。 */
export function appendReasoningDelta(segments, summaryIndex, delta) {
...
function skillDraftThreadSettings(cwd) {
    return {
...
function codexInput(prompt, images, skill) {
...
function mentionsSkill(prompt, name) {
...
function normalizeItem(item) {
    ...
    if (value.type === "collabToolCall" || value.type === "collabAgentToolCall")
...
function mergeDefinedRecord(started, completed) {
