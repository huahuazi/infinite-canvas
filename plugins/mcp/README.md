...
- **HTTP 服务**（默认模式）：`npx -y @tigerowo/infinite-canvas-agent`，提供网页画布连接所需地址与 token
...
```bash
npx -y @tigerowo/infinite-canvas-agent mcp
```
...
| Agent | 注册命令 | 工具前缀 |
| --- | --- | --- |
| Codex | `codex mcp add infinite-canvas -- npx -y @tigerowo/infinite-canvas-agent mcp` | 直接为 `name`（如 `infinite-canvas_get_canvas_summary`）或无前缀 |
| Claude Code | `claude mcp add infinite-canvas -- npx -y @tigerowo/infinite-canvas-agent mcp` | `mcp__infinite-canvas__get_canvas_summary` |
| Gemini CLI | `gemini mcp add infinite-canvas -- npx -y @tigerowo/infinite-canvas-agent mcp` | `mcp__infinite-canvas__get_canvas_summary` |
| Cursor | 设置 → MCP → Add → 选择 `command` 类型，填 `npx -y @tigerowo/infinite-canvas-agent mcp` | `mcp__infinite-canvas__get_canvas_summary` |
| 其他支持 MCP 的 Agent | 使用各自 `mcp add`（或 `mcp --connect`）命令，命令体固定为 `npx -y @tigerowo/infinite-canvas-agent mcp` | 见各 Agent 文档 |
