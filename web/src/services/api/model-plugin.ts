...
/**
 * Run a user-authored model call script as an async function body with flat locals (see getPluginVariables):
...
export function getPluginReturn(capability: ModelCapability) {
    const messageByCapability: Record<ModelCapability, string> = {
    ...
    return messageByCapability[capability];
}
