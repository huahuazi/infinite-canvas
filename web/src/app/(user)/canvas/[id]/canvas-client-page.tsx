...
function InfiniteCanvasPage({ projectId }: { projectId: string }) {
    ...
    const agentEffectiveConfig = useMemo(
        () => ({
            ...
            textModel: resolvedAgentConfig.textModel || effectiveConfig.textModel || effectiveConfig.model,
            textChannelId: resolvedAgentConfig.textChannelId || effectiveConfig.textChannelId || effectiveConfig.activeChannelId,
            imageModel: resolvedAgentConfig.imageModel || effectiveConfig.imageModel || effectiveConfig.model,
            imageChannelId: resolvedAgentConfig.imageChannelId || effectiveConfig.imageChannelId || effectiveConfig.activeChannelId,
            videoModel: resolvedAgentConfig.videoModel || effectiveConfig.videoModel || effectiveConfig.model,
            videoChannelId: resolvedAgentConfig.videoChannelId || effectiveConfig.videoChannelId || effectiveConfig.activeChannelId,
            audioModel: resolvedAgentConfig.audioModel || effectiveConfig.audioModel,
            audioChannelId: resolvedAgentConfig.audioChannelId || effectiveConfig.audioChannelId || effectiveConfig.activeChannelId,
            count: "1",
            canvasImageCount: "1",
        }),
    ...
    useEffect(() => {
        const unsubscribe = subscribeWorkflowImages(async (images: CanvasWorkflowImage[]) => {
            ...
            for (const [index, image] of images.entries()) {
                ...
                await insertAssistantImageRef.current({ id: `workflow-${Date.now()}-${index}`, dataUrl: url, prompt: image.prompt || image.workflowName || "工作流结果", storageKey: image.storageKey || "", source: "asset" }, position);
            }
        });
        return unsubscribe;
    ...
    return (
        ...
        >
            ...
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                {explodeNode?.metadata?.content ? (
                    <CanvasNodeExplodeDialog
                        dataUrl={explodeNode.metadata.content}
                        open={Boolean(explodeNode)}
                        config={buildGenerationConfig(effectiveConfig, explodeNode, "image")}
